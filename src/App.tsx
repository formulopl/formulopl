...
import {
  ...
  MLXConfig,
  LLMProvider,
} from './services/threeAgentSystem';
...
const Icon = ({ type, label }: { type: string; label?: string }) => {
  const icons: Record<string, { svg: string; color: string }> = {
    graduation: {
      svg: '<path d="M12 3L1 9l4 2.18v4L12 19l7-3.82v-4l2-1.09V17h2V9L12 3zm6.82 6L12 12.72 5.18 9 12 5.28 18.82 9zM17 15.99l-5 2.73-5-2.73v-3.72L12 15l5-2.73v3.72z"/>',
      color: '#6b46c1',
    },
    books: {
      svg: '<path d="M21 5c-1.11-.35-2.33-.5-3.5-.5-1.95 0-4.05.4-5.5 1.5-1.45-1.1-3.55-1.5-5.5-1.5S2.45 4.9 1 6v14.65c0 .25.25.5.5.5.1 0 .15-.05.25-.05C3.1 20.45 5.05 20 6.5 20c1.95 0 4.05.4 5.5 1.5 1.35-.85 3.8-1.5 5.5-1.5 1.65 0 3.35.3 4.75 1.05.1.05.15.05.25.05.25 0 .5-.25.5-.5V6c-.6-.45-1.25-.75-2-1zm0 13.5c-1.1-.35-2.3-.5-3.5-.5-1.7 0-4.15.65-5.5 1.5V8c1.35-.85 3.8-1.5 5.5-1.5 1.2 0 2.4.15 3.5.5v11.5z"/>',
      color: '#2563eb',
    },
    camera: {
    ...
    plug: {
      svg: '<path d="M16 9V4.5C16 3.12 17.12 2 18.5 2S21 3.12 21 4.5V9h-5zm-8 0V4.5C8 3.12 6.88 2 5.5 2S3 3.12 3 4.5V9h5zm11.5 2h-15C3.67 11 3 11.67 3 12.5V13c0 3.47 2.61 6.34 6 6.92V22h6v-2.08c3.39-.58 6-3.45 6-6.92v-.5c0-.83-.67-1.5-1.5-1.5z"/>',
      color: '#10b981',
    },
    target: {
      svg: '<path d="M12 2C6.49 2 2 6.49 2 12s4.49 10 10 10 10-4.49 10-10S17.51 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8zm3-8c0 1.66-1.34 3-3 3s-3-1.34-3-3 1.34-3 3-3 3 1.34 3 3z"/>',
      color: '#dc2626',
    },
    wave: {
      svg: '<path d="M7 11.5c0-.83.67-1.5 1.5-1.5S10 10.67 10 11.5V12h4v-.5c0-.83.67-1.5 1.5-1.5s1.5.67 1.5 1.5V15c0 2.21-1.79 4-4 4h-2c-2.21 0-4-1.79-4-4v-3.5zM12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8z"/>',
      color: '#f59e0b',
    },
    brain: {
    ...
    microscope: {
      svg: '<path d="M13 11.33L18 18H6l5-6.67V6.83c-.85-.3-1.53-1-1.83-1.83h3.66c-.3.83-.98 1.53-1.83 1.83v4.5zM15.32 3a2.98 2.98 0 01-1.56 2.2 2.98 2.98 0 01-3.52 0A2.98 2.98 0 018.68 3H6v2h1.09A5 5 0 007 7.17V11l-4 5.33V20h18v-3.67L17 11V7.17A5 5 0 0016.91 5H18V3h-2.68z"/>',
      color: '#6366f1',
    },
    bulb: {
    ...
    error: {
      svg: '<path d="M12 2C6.47 2 2 6.47 2 12s4.47 10 10 10 10-4.47 10-10S17.53 2 12 2zm5 13.59L15.59 17 12 13.41 8.41 17 7 15.59 10.59 12 7 8.41 8.41 7 12 10.59 15.59 7 17 8.41 13.41 12 17 15.59z"/>',
      color: '#dc2626',
    },
    success: {
...
const MCP_PROXY_URL =
  import.meta.env.VITE_MCP_PROXY_URL || 'http://localhost:3001';
const DEFAULT_REMOTE_MODEL =
  import.meta.env.VITE_REMOTE_MODEL || 'speakleash/Bielik-11B-v3.0-Instruct';
const DEFAULT_REMOTE_API_URL =
  import.meta.env.VITE_REMOTE_API_URL || 'https://llmlab.plgrid.pl/api';
const PRECONFIGURED_PROVIDER = import.meta.env.VITE_LLM_PROVIDER as
  | string
  | undefined;
...
function App() {
  ...
  const [llmProvider, setLlmProvider] = useState<LLMProvider>(
    PRECONFIGURED_PROVIDER === 'remote' ? 'remote' : 'ollama',
  );
  ...
  const [activePage, setActivePage] = useState<
    'chat' | 'formulas' | 'quiz' | 'plan' | 'notebook' | 'stats'
  >(() => {
  ...
  const [notebookCount, setNotebookCount] = useState<number>(() =>
    countEntries(),
  );
  ...
  const navigateTo = (
    page: 'chat' | 'formulas' | 'quiz' | 'plan' | 'notebook' | 'stats',
  ) => {
    setActivePage(page);
    ...
    const autoDetect = async () => {
      ...
      if (PRECONFIGURED_PROVIDER === 'remote') {
        ...
        try {
          ...
          /* MCP connection will be retried on first message */
        }
      ...
      try {
        const mlxRes = await fetch('http://localhost:8011/v1/models', {
        ...
        /* MLX not running, continue to Ollama check */
      }
      ...
      try {
        ...
        if (ollamaData?.models?.length > 0) {
          const bielikModel = ollamaData.models.find((m: { name: string }) =>
            /bielik/i.test(m.name),
          );
      ...
      try {
        ...
        /* MCP connection will be retried */
      }
      const newChatId = ChatHistoryService.generateChatId();
      setCurrentChatId(newChatId);
  ...
  useEffect(() => {
    if (messages.length > 0 && currentChatId) {
      const session: ChatSession = {
        ...
        messages,
        createdAt:
          messages[0]?.timestamp.toISOString() || new Date().toISOString(),
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      alert(
        'Wystapil blad podczas komunikacji z agentem. Sprawdz klucz API i polaczenie.',
      );
    } finally {
  ...
  const handleExportToPNG = async () => {
    ...
    try {
      ...
      messageElements.forEach((msgElement) => {
        ...
        msgClone.style.backgroundColor = msgClone.classList.contains('user')
          ? '#667eea'
          : '#f0f0f0';
        ...
        msgClone.querySelectorAll('.katex').forEach((katexEl) => {
          ...
          const texSource =
            annotation?.textContent || katexEl.textContent || '';
  ...
  return (
    <div className='app-shell'>
      <aside
        className={`app-rail ${railCollapsed ? 'collapsed' : ''}`}
        aria-label='Pasek narzędzi'
      >
        <button
          className='rail-logo'
          onClick={() => navigateTo('chat')}
          title='Formulo — strona główna'
        ...
        <div className='rail-actions'>
          ...
          <button
            className='rail-btn rail-btn-export'
            onClick={handleExportToPNG}
            disabled={messages.length === 0}
            title='Eksportuj rozmowę do PNG'
            aria-label='Eksport do PNG'
          >
            <Icon type='camera' />
          </button>
        </div>
      ...
      <div className='app-container'>
        {showWarmupBanner && (
          ...
          >
            ...
            {warmStatus?.bedrock.status === 'warming' &&
              warmStatus.bedrock.warmingForSec != null && (
                <> ({warmStatus.bedrock.warmingForSec}s)</>
              )}
            . Pierwsze zapytanie po dłuższej przerwie może potrwać 5-10 minut.
            Kolejne będą szybkie. Bedrock CMI ładuje 11B parametrów Bielika do
            GPU.
          </div>
        )}
        <header className='app-header'>
          ...
          <div className='header-controls'>
            ...
            <div className='page-tabs'>
              ...
              <button
                className={`page-tab ${activePage === 'plan' ? 'active' : ''}`}
                onClick={() => navigateTo('plan')}
                title='Plan nauki'
              >
                <span className='tab-icon' aria-hidden='true'>
              ...
              <button
                className={`page-tab ${activePage === 'quiz' ? 'active' : ''}`}
              ...
              <button
                className={`page-tab ${activePage === 'formulas' ? 'active' : ''}`}
              ...
              <button
                className={`page-tab ${activePage === 'notebook' ? 'active' : ''}`}
                onClick={() => navigateTo('notebook')}
                title='Notatnik Matematyczny'
              >
                <span className='tab-icon' aria-hidden='true'>
                  <svg
                    viewBox='0 0 24 24'
              ...
              <button
                className={`page-tab ${activePage === 'stats' ? 'active' : ''}`}
        ...
        {activePage === 'formulas' ? (
          ...
          <div className='chat-container'>
            <div
              className={`messages-container${messages.length === 0 ? ' messages-container--landing' : ''}`}
            >
              {messages.length === 0 ? (
                              ...
                              msg.agentName === 'Agent Analityczny'
                                ...
                                : msg.agentName === 'Agent Wykonawczy'
                                  ...
                                  : msg.agentName === 'Agent Weryfikujący'
                                    ? 'target'
                                    : msg.agentName === 'Agent Formalizujący'
                                      ? 'microscope'
                                      : 'robot'
                            }
                          />{' '}
                          ...
                          <div className='tool-calls'>
                            {msg.toolCalls.map((tc) => (
                              <div key={tc.id} className='tool-call'>
                                <Icon type='wrench' /> Używam narzędzia:{' '}
                                <code>{tc.name}</code>
                                <details
                                  style={{
                                    marginTop: '0.5em',
                        ...
                        {msg.toolResults && msg.toolResults.length > 0 && (
                          <div className='tool-results'>
                            {msg.toolResults.map((tr, idx) => (
                              ...
                              >
                                <Icon type={tr.isError ? 'error' : 'success'} />{' '}
                                {tr.isError ? 'Błąd' : 'Wynik'}{' '}
                                <code>{tr.toolName}</code>:{' '}
                                {tr.isError &&
                                (tr.result.includes('Traceback') ||
            ...
            <div className='input-container'>
              ...
              <textarea
                ...
                placeholder={
                  isMobile
                    ? 'Wpisz zadanie...'
                    : 'Wpisz zadanie matematyczne... (Enter aby wysłać, Shift+Enter dla nowej linii)'
                }
            ...
            {messages.length === 0 && (
              <section className='examples-section examples-below-input'>
                <h4 className='section-title'>
                  Wybierz przykład, aby zobaczyć rozwiązanie krok po kroku
                </h4>
        ...
        <footer className='app-footer'>
          ...
          <a
            href='https://github.com/formulopl/formulopl'
            target='_blank'
            rel='noopener noreferrer'
          ...
          <span>
            Powered by{' '}
            <a
              href='https://huggingface.co/speakleash/Bielik-11B-v3.0-Instruct'
              target='_blank'
              rel='noopener noreferrer'
            >
              Bielik v3 11B
          ...
          <span className='footer-legal'>
            ...
            <a
              ...
              onClick={(e) => {
                e.preventDefault();
                localStorage.removeItem('formulo-cookie-consent');
