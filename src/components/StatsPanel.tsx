...
const TopicBars: React.FC<{ topicStats: Record<string, number> }> = ({
  topicStats,
}) => {
  ...
  if (entries.length === 0) {
    return (
      <div className='stats-empty-topics'>
        <span>
          Brak danych – rozwiąż kilka zadań, żeby zobaczyć rozkład tematów 📊
        </span>
      </div>
    );
  }
...
const BadgeWall: React.FC<{ state: GamificationState }> = ({ state }) => {
  ...
  return (
    <div className='badge-wall'>
      ...
      {earned.length === 0 && (
        <p className='badge-wall-empty'>
          Jeszcze nie masz odznak. Zacznij rozwiązywać zadania!
        </p>
      )}
    </div>
  );
...
function getReturnMessage(
  ...
  if (daysDiff <= 3) {
    return {
      text: `Byłeś tu ${daysDiff} dni temu. Wróciłeś! Dokończ serię.`,
      emoji: '⚡',
    };
  }
  if (daysDiff <= 14) {
    return {
      text: `Przerwa ${daysDiff} dni. Czas na powrót – matura nie czeka!`,
      emoji: '📚',
    };
  }
  return {
    text: `Długa przerwa (${daysDiff} dni). Zacznij od nowa – pierwsze kroki są najważniejsze.`,
    emoji: '🚀',
  };
}
    ...
    <div className='stats-panel'>
        ...
        <StatCard
          ...
          sub={
            state.perfectQuizzes > 0
              ? `${state.perfectQuizzes} idealnych quizów`
              : undefined
          }
        />
      </div>
      ...
      <div className='stats-level-bar-wrapper'>
        <div className='stats-level-bar-labels'>
          <span>
            Poziom {levelInfo.level}: <strong>{levelInfo.name}</strong>
          </span>
      ...
      <section className='stats-section'>
        ...
        {Object.keys(state.topicStats).length === 0 && (
          <p className='stats-tip'>
            💡 Wskazówka: możesz śledzić tematy wpisując np. „Rozwiąż zadanie z
            geometrii"
          </p>
        )}
      </section>
      ...
      <section className='stats-section'>
        <h3 className='stats-section-title'>
          🏅 Odznaki ({earnedBadgesCount}/{BADGES.length})
        </h3>
