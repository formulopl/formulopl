...
export function QuizMode({
  ...
  const [selectedTopic, setSelectedTopic] =
    useState<string>('Wszystkie tematy');
  ...
  const fetchQuiz = async () => {
    ...
    try {
      const topic =
        selectedTopic === 'Wszystkie tematy' ? 'all' : selectedTopic;
      const response = await fetch(
        `/api/quiz?level=${selectedLevel}&count=5&topic=${topic}`,
      );
      ...
      const normalized = (data.questions || []).map(
        (q: Record<string, unknown>) => ({
          ...
          task_number:
            (q.metadata as Record<string, unknown>)?.task_number ??
            q.task_number,
        }),
      );
  ...
  const finishQuiz = async () => {
    try {
      ...
      const correct =
        data.correct_count ||
        answers.filter(
          (_, idx) => userAnswers.get(idx) === questions[idx].correct_answer,
        ).length;
      const wrongQuestions = questions
        .map((_, idx) => idx)
        .filter(
          (idx) => userAnswers.get(idx) !== questions[idx].correct_answer,
        );
  ...
  return (
    <div className='quiz-screen'>
      <div className='quiz-container'>
        ...
        <div
          className={`question-card ${answered ? (isCorrect ? 'correct' : 'incorrect') : ''}`}
        >
            ...
            >
              {normalizeOptions(question.options).length === 0 ? (
                <>
                  ...
                  <div className='feedback-open'>
                    <span className='feedback-text'>
                      <strong>Odpowiedź modelowa:</strong>
                    </span>
                    <span
                      className='feedback-model-answer'
                      dangerouslySetInnerHTML={{
                        __html: renderLatex(question.correct_answer),
                      }}
                    />
                    <button
                ...
                <>
                  <span className='feedback-icon'>✗</span>
                  <span className='feedback-text'>
                    Niepoprawnie. Prawidłowa odpowiedź:{' '}
                ...
                <div key={idx} className='wrong-item'>
                  <div className='wrong-question-info'>
                    ...
                    <span
                      ...
                      dangerouslySetInnerHTML={{
                        __html: renderLatex(questions[idx].question),
                      }}
                    />
                  </div>
                  <button
